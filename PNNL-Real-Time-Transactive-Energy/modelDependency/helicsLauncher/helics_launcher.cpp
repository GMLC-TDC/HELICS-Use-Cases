...
enum loglevel_e
    {logWARNING=1, logINFO=2, logDEBUG=3, logDEBUG1=4, logDEBUG2=5, logDEBUG3=6, logDEBUG4=7};
...
std::string federate_ip;
std::string helics_port = "7777";
std::string core_tick = "10s";
std::string core_timeout = "100s";
...
int main(int argc, char **argv)
{
    ...
    log_level_export = getenv("HELICS_LAUNCHER_LOG_LEVEL");
    ...
    if (!log_level_export) {
        ...
        launcher_log_level = logINFO;
        LINFO << "The HELICS_LAUNCHER_LOG_LEVEL was set to an unknown value";
    }
    ...
    // translate the hostname to an IP that HELICS can understand
    temp_ip = gethostbyname(host_name);
    federate_ip = "tcp://";
    federate_ip += std::string(inet_ntoa(**(in_addr**)temp_ip->h_addr_list));
    ...
    */
    ...
    temp_ip = gethostbyname(host_name);
    broker_ip = "tcp://";
    broker_ip += std::string(inet_ntoa(**(in_addr**)temp_ip->h_addr_list));

    LDEBUG << "broker -> " << broker_ip << " federate -> " << federate_ip;
    ...
    for (size_t i=0; i<commands.size(); ++i) {
        ...
        /* this env var is used by a special test program only */
        {
            ...
            val += "_";
            val += to_string(i);
            setenv("HELICS_LAUNCHER_RANK", val.c_str(), 1);
            LDEBUG << "HELICS_LAUNCHER_RANK=" << val;
        }
    ...
    if (0 == world_rank) {
        ...
        std::ifstream src("helics.out", std::ios::binary);
...
pid_t run_command(std::string command)
{
    ...
    if (-1 == cpid) {
        ...
        /* this is the child */
        int retval;
        int fd = open("helics.out", O_RDWR|O_CREAT, S_IRUSR|S_IWUSR);
        if (-1 == fd) {
            ERRNO << "open(helics.out)";
            _exit(EXIT_FAILURE);
        }
...
std::vector<std::string> parse_input_file(const char *filename)
{
    ...
    if (0 == world_rank) {
        std::ostringstream os;
        //os << "pwd helics_broker " << total;
        os << "pwd helics_broker" << " --federates " << total << " --log_level " << launcher_log_level << " --interface " << federate_ip << ":" << helics_port << " --tick=" << core_tick << " --timeout=" << core_timeout;
