...
/*
   ...
   - nFNCSfeeders = number of feeders/GridLAB-Ds; more than one distribution network could be connected to the same bus
...
void read_model_data(char *file_name, int nbrows, int nbcolumns, int ngrows, int ngcolumns,
              ...
              int ncrows, int nccolumns, int nFNCSbuses, int nFNCSfeeders, int noffgelem,
              double *baseMVA, vector<double> &bus, vector<double> &gen,
              vector<double> &branch, vector<double> &area, vector<double> &costs, vector<int> &BusFNCS,
              vector<string> &FeederNameFNCS, vector<int> &FeederBusFNCS,
...
bool read_BusFNCS = 0, read_offlineGenBus = 0, read_FeederNameFNCS = 0;
  ...
  while (data_file.good()) { // this will test the EOF mark
    ...
    if (curr_line[0] != '%') {
      ...
      if (strncmp(&curr_line[0], "mpc.BusCoSim =", 14) == 0) {
        ...
        read_BusFNCS = 1; }
      if (read_BusFNCS == 1 & strncmp(&curr_line[0], "mpc.BusCoSim =", 14) != 0){
        if (ind < nFNCSbuses){
          read_BusFNCS = 1;
      ...
      // ================== READING FEEDER NAMES FOR FNCS COMMUNICATION - T&D level =========================================              
      if (strncmp(&curr_line[0], "mpc.FeederNameCoSim =", 21) == 0) {
        LDEBUG << "Reading FNCS FEEDER NAMES ...................." ;
        read_FeederNameFNCS = 1; }
      if (read_FeederNameFNCS == 1 & strncmp(&curr_line[0], "mpc.FeederNameCoSim =", 21) != 0){
        if (ind < nFNCSfeeders){
          read_FeederNameFNCS = 1;
          char temp[50];
          sscanf(&curr_line[0], "%s %d %*s", temp, &FeederBusFNCS[ind]);
          FeederNameFNCS[ind] = temp;
          ind += 1;
        }
        else {
          read_FeederNameFNCS = 0;
          ind = 0;
        }
      }
