...
// =================================================================
// Laurentiu Dan Marinovici - 2017/06/27; modified 2018/04/20
// =================================================================
void getDispLoad(string lookupKey, double *maxDispLoad)
{
  string lookupKval;
  // get the maximum dispatchable load, if exists, from the corresponding global key published by the DSO
  lookupKval = fncs::get_value(lookupKey);

  if (!lookupKval.empty())
  {
    sscanf(&lookupKval[0], "%lf", maxDispLoad); // parse the value to get the numeric maximum dispatchable load; should be in MW
  }
  else
  {
    maxDispLoad = 0; // no value set, so putting it to zero
  }
}
...
// =================================================================
// Laurentiu Dan Marinovici - 2018/04/20
// =================================================================
void getUnrespLoad(string lookupKey, double *unrespLoad)
{
  string lookupKval;
  // get the value of the unresponsive load, if exists, from the corresponding global key published by the DSO
  lookupKval = fncs::get_value(lookupKey);

  if (!lookupKval.empty())
  {
    sscanf(&lookupKval[0], "%lf", unrespLoad); // parse the value to get the numeric unresponsive load value; should be in MW
  }
  else
  {
    unrespLoad = 0;
