...
from supportFunctions.realTimeTransactiveEnergy import helicsConfiguration
from supportFunctions.realTimeTransactiveEnergy import createDistributionFeeder
from supportFunctions.realTimeTransactiveEnergy import controlConfiguration
from pathlib import Path
import time, os, shutil, math, tqdm, multiprocessing, platform, sys, random
...
# get list of feeders that should have control activated
advancedControlList = random.sample(range(0,distributionSystemNumber), math.floor(distributionSystemNumber*controlConfig['transactivepenetration']))
...
for key, number in enumerate(distributionNumberVector):
	for keyOffset in range(0, int(number)):
		populationDict[count] = {'originalName': os.path.splitext(list(inputGLM.keys())[key])[0],
								 ...
								 'add_real_time_transactive_control': False}
		#print 'key offset -> ', keyOffset, 'control -> ',advancedControlBool
		if count in advancedControlList:
			populationDict[count]['add_real_time_transactive_control'] = True
		count += 1
...
if success:
	...
	# add the DSO HELICS configuration files
	if useFlags['use_HELICS']:
		helicsConfiguration.dso_config(populationDict)
