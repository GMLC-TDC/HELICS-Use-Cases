...
int http_xml_request(HTTPCNX *http,char *uri)
{
	...
	char arg1[10240]="", arg2[10240]="";
	int nargs = sscanf(uri,"%10239[^/=\r\n]/%10239[^\r\n=]",arg1,arg2);
	char *value = strchr(uri,'=');
	char buffer[10240]="";
	OBJECT *obj=NULL;
	char *id;
	char *ptr;
	int event_count=0,loop;
	char obj_name[100][128];
  char obj_property[100][128];
  char obj_value[100][128];
  char qry[10240];
  
  strncpy(qry,uri,sizeof(qry));
	
	if(nargs>1){
    ptr=strtok(uri,"/");
    if((ptr!=NULL)){
      strncpy(obj_name[event_count],ptr,sizeof(obj_name[event_count]));
      ptr=strtok(NULL,"/");
      strncpy(obj_property[event_count],ptr,sizeof(obj_property[event_count]));
      ptr=strtok(NULL,"/");
      strncpy(obj_value[event_count],ptr,sizeof(obj_value[event_count]));
      event_count++;
    }
...
/*	  copy objects*/
      strncpy(arg1,obj_name[loop],sizeof(obj_name[loop]));
      strncpy(arg2,obj_property[loop],sizeof(obj_property[loop]));

	    /* get global variable */
	    if(strcmp(obj_name[loop],"global")==0){

		    /* find the variable */
		    if (global_getvar(obj_property[loop],buffer,sizeof(buffer))==NULL)
		    {
		    ...
		    /* assignment, if any */
		    if (strcmp(obj_value[loop],"none")!=0) global_setvar(obj_property[loop],obj_value[loop]);
...
/*		    http_type(http,"text/xml");*/
	    ...
	    /* get object property */
		    ...
		    /* assignment, if any */
		    if ( (strcmp(obj_value[loop],"none")!=0) && !object_set_value_by_name(obj,arg2,obj_value[loop]) )
		    {
		    ...
		    /* post the response */
    		http_format(http,"%s,", arg1);
    		http_format(http,"%s,", arg2);
		    http_format(http,"%s,", http_unquote(buffer));
...
/*	#####experimental*/
  memset(http->query,0,sizeof(http->query));
