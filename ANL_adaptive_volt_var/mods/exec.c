...
/*socket conn variable*/
int socket_desc,nPass=0;


/* #########################some user defined socket functions for connection ###########*/
/*create a socket*/

int socket_open(char *hostIP,int hostPort){
...
int socket_send(int socket_desc, char *qry)
{
  int flg,send_flg=0;
/*SAMPLE CALL: socket_conn(socket_desc, server_reply,"GET /xml/Node632/voltage_A HTTP/1.1");*/
	
	while(send_flg==0){
	  send_flg=send(socket_desc , qry , strlen(qry) , 0);
	}
	//Send some data
	if(send_flg < 0)
	{
...
/*socket connection function*/
int socket_recv(int socket_desc, char *server_reply)
{
/*SAMPLE CALL: socket_conn(socket_desc, server_reply,"GET /xml/Node632/voltage_A HTTP/1.1");*/
int read_size=0;

while(read_size==0){
  read_size=recv(socket_desc, server_reply , 2048 , 0);
}
...
STATUS exec_start(void)
{
	...
	TRY {
			...
			&& exec_getexitcode()==XC_SUCCESS ) 
		{
			...
			/*open socket*/
			if(nPass==0){
				socket_desc=socket_open(ipAddr,global_client_portnum);
			}
			nPass++;
			rf=socket_recv(socket_desc, server_reply);
			...
			/* ################# TS sync ####################*/
				...
				sprintf(sync_msg,"%d",global_nr_convergence_flag);
				sf=socket_send(socket_desc,sync_msg);
