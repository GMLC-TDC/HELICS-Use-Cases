...
freqcalc(k,t,0); % included by Rui
...
% initialize helics--------------------------
global freq_pst freq_received 
global helics_send_freq  helics_recv_freq


initstring = '2 --name=mainbroker';
fedinitstring = '--broker=mainbroker --federates=1';
helics_hstep = 0.005;   % need to change to make it same as the data file simulation time
...
% Set Federate name
status = helics.helicsFederateInfoSetFederateName(fedinfo, 'PST');
...
status = helics.helicsFederateInfoSetTimeDelta(fedinfo, helics_hstep);

status = helics.helicsFederateInfoSetLoggingLevel(fedinfo, 1);

% Create value federate
vfed = helics.helicsCreateCombinationFederate(fedinfo);
display('PST SIMU: combination federate created');

% Register the publication
% pubvmag = helics.helicsFederateRegisterEndpoint(vfed, 'VMAG', '');
pubvang = helics.helicsFederateRegisterEndpoint(vfed, 'send_freq', '');
subpreal = helics.helicsFederateRegisterEndpoint(vfed, 'recv_freq', '');
% subqimag = helics.helicsFederateRegisterEndpoint(vfed, 'QIMAG', '');
display('pst simu: Publication and subscription registered');

% Enter execution mode
status = helics.helicsFederateEnterExecutionMode(vfed);
display('LOAD SIMULATOR: Entering execution mode');

helics_currentTime = 0;
helics_simulation_total_time = 10;
helics_istep = 1; % integer

%helics_sub_keys = {'LD1', 'LD2'};
%helics_pub_keys = {'VT1', 'VT2'};

helics_sub_keys = {'send_freq'};
helics_pub_keys = { 'recv_freq'};


helics_nsteps = helics_simulation_total_time/helics_hstep;
helics_send_freq = zeros(1, helics_nsteps);
helics_recvfreq = zeros(1, helics_nsteps);
...
while (kt<=ktmax)
   ...
   for k = k_start:k_end
      ...
      freqcalc(k,t,1); %included by Rui
      ...
      freqcalc(j,t,1);  % Included by Rui
      vex(:,j)=vex(:,k);
      cur_ord(:,j) = cur_ord(:,k);
      % calculate the new value of bus angles rectifier user defined control
      if ndcr_ud~=0
      ...
      % publish messages to helics
      
      %for icount = 1:nhelics_pub_count
      
      
      val_pubfreq = freq_pst;
            

      send_time =       (helics_istep*helics_hstep); % it seems ns3 has 20 times time difference with pst
      status = helics.helicsEndpointSendEventRaw(pubvang, 'ns3/endpoint1', num2str(val_pubfreq), send_time);
      display(['PST SIMULATOR: Sending freq value = ', '', num2str(val_pubfreq), ' to LOAD SENDER at time',num2str(send_time)]);
      
      % sub messages to helics
      
      if helics_istep == 1
         helics_nextTime = helics.helicsFederateRequestTime(vfed, send_time);
      else
%          helics_nextTime = helics.helicsFederateRequestTime(vfed, helics_istep-1);
         helics_nextTime = helics.helicsFederateRequestTime(vfed, send_time);

      end
      
      isupdated1 = helics.helicsEndpointHasMessage(subpreal);
      
      if isupdated1 == 1 
        message1 = helics.helicsEndpointGetMessage(subpreal);
        display(['PST SIMULATOR: Received freq value = ', '', message1.data, '', ' at time ', '', num2str(send_time), ' from ns3']);
        tmppreal = str2double(message1.data);
        freq_received = tmppreal;
      end
      

      
  
                helics_send_freq(k) = val_pubfreq;
                helics_recv_freq(k) = tmppreal;
                
     
     
        helics_istep = helics_istep +1;
        %helics_currentTime = helics_nextTime
...
display('LOAD SIMULATOR: Federate finalized');
...
display('PST SIMULATOR: Broker disconnected');
% helics_finalize finished
...
save system_16mach_nogov.mat t mac_ang mac_spd pmech pelect bus_v
